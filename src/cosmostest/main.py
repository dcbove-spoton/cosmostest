...
def authenticate(credentials: Annotated[HTTPBasicCredentials, Depends(security)]) -> None:
    ...
    username_matches = secrets.compare_digest(credentials.username.encode("utf-8"), b"test")
    password_matches = secrets.compare_digest(credentials.password.encode("utf-8"), b"test")
